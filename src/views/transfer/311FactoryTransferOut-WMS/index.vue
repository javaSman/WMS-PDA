  ...
  methods: {
    ...
    comfirAdd(data) {
      ...
      let arr = data.map((item) => ({
        ...
        wmsOutStockList: {
          WHId: `${originObj.werks}-${originObj.lgort}`, // 仓库
          boxID: this.boxInfo.boxID, // 所属载具
          locationID: this.barcodeInfo.locationNo, // 所属货位
          // receiveBoxID: this.boxInfo.boxID, // 目标载具
          // TargetBarcode: this.barcodeInfo.OBJNR, // 目标条码
          TargetBarcodeQuantity: this.form.qty, // 转入目标条码数量
          targetBoxID: this.boxInfo.boxID, // 目标载具
          // targetLocationID: this.boxInfo.locationNo, // 目标货位
          materialID: this.barcodeInfo.MATNR, // 物料号
          materialDesc: this.barcodeInfo.MAKTX, // 物料描述
          quantity: this.form.qty, // 数量
          unitID: this.barcodeInfo.GMEIN, // 单位
          barcode: '', // 物料条码
    ...
    async handleConfirm() {
      ...
      try {
        let result = {
          ...
          CardName: originObj.cardname, // 第二次登录姓名
          CardNo: originObj.cardno, // 第二次登录工号
          // WarehouseNo: `${originObj.werks}-${originObj.lgort}`, // 仓库编号
          // TargetWarehouseNo: this.targetWharehouse, // 目标仓库编号
          zxstXmbeStruList: this.result.map((item) => item.zxstXmbeStruList), // Sap过账明细
          wmsOutStockList: this.result.map((item) => item.wmsOutStockList) // WMS过账明细
        }
        console.log(result)
        ...
        this.loading = true
        let res = await WMSAPI_WCF.post('9037Wcf/WarehouseService.svc/rest/OutStock', result)
