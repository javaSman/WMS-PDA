<template>
  <div>
    ...
    <InputModal ref="inputNumberRef" :visible.sync="inputNumberVisible" @inputNumber="inputNumberHandler" />
  </div>
  <!-- <ToggleFormWMSVue
...
import InputModal from '@/views/businessComponents/InputModal.vue'
...
// let originData = [] // 保留一份原始数据
export default {
  name: 'Machined105GoodToWarehouseWMS',
  components: {
    // ToggleFormWMSVue
    FormVue,
    VirtualTable,
  ...
  data() {
    return {
      ...
      inputNumberVisible: false // 数量输入弹窗
    }
  },
  computed: {
  ...
  methods: {
    ...
    async barcodeEnter(val) {
      ...
      if (this.list.length > 0) {
        ...
        if (target.length > 0) {
          ...
          // tips: 为了支持多次扫码呼出修改数量弹窗，所以利用set去重实现
        ...
        try {
          ...
          if (res && res.success) {
            let _data = res.data
            // 判断这里是否都是机加件的物料，2或字母开头
            if (_data.some((item) => !/[a-zA-Z]|^2/.test(this.computedMaterialType(item)))) {
              _showFailToast('非机加件的物料不允许在这里过账')
              return
            }
            if (_data.length === 0) {
              _showFailToast('暂无数据')
    ...
    async handleConfirm() {
      ...
      try {
        ...
        // tips: 针对提交给wms的数据重新处理一下，因为erfmg也是有值的，所以用户修改了qcqty和tcqty的时候不生效，所以，这两个修改的值也要同步修改erfmg才行
        // tips: 被修改的数据源在sap列表中有保留，所以从sap列表中映射过来即可
        _data.wmsPostInList = _data.wmsPostInList.map((item, index) => ({
          ...item,
          erfmg:
            Number(_data.zxstXmbeStruList[index].qcqty) > 0
              ? Number(_data.zxstXmbeStruList[index].qcqty)
              : Number(_data.zxstXmbeStruList[index].tcqty),
          quantity:
            Number(_data.zxstXmbeStruList[index].qcqty) > 0
              ? Number(_data.zxstXmbeStruList[index].qcqty)
              : Number(_data.zxstXmbeStruList[index].tcqty)
        }))
        // tips:sap的数据保持和扫描时的一样
        // let _originSelect = originData.filter((item) => this.selectedList.includes(item.uuid))
        // _data.zxstXmbeStruList = _originSelect.map((item) => ({
        //   ...item,
    ...
    // tips:因为扫描勾选是会自动置顶的，所以直接修改表格的第一项就是目标项目了
    inputNumberHandler(val) {
      let barcodeRef = this.getVueComponentByProp('imBarcode')
      let target = this.list[0]
      // 哪一项有值就改哪一项，良品数和特采数
      if (Number(target.qcqty) > 0) {
        target.qcqty = val
      } else {
        target.tcqty = val
      }
      this.$set(this.list, 0, target)
      this.inputNumberVisible = false
      barcodeRef.focus()
    },
    // 判断物料类型
    computedMaterialType(item) {
      if (item.matnr) {
        return item.matnr.replace(/^0+/gi, '')
      } else {
        return item.idnrk.replace(/^0+/gi, '')
      }
    }
  }
}
