    ...
    async boxIDEnter(val) {
      ...
      try {
        ...
        if (res.success) {
          ...
          this.$set(this.form, 'targetLocationID', res.box.locationNo)
    ...
    async handleConfirm() {
      ...
      if (_selection.some((item) => item.projectNo !== _target.projectNo || item.stationNo !== _target.stationNo || item.batch !== _target.batch)) {
        await this.$dialog
          .confirm({
            title: '重要提示',
            message: `所选项的【项目-${_target.projectNo}】、【工位-${_target.stationNo}】、【批次-${_target.batch}】存在不同项，无法过账`
          })
          .then(() => {
            return
            // _handler(_that)
