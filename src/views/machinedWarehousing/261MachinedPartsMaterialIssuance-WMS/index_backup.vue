...
import { GetBoxDetail, GetLocationInfo, GetBoxInfoByBoxId, GetUserWarehouse } from '@/api/common'
  ...
  methods: {
    ...
    initConfig() {
      this.formList.forEach((item) => {
        ...
        //       { validator: this.validateErfmg, message: '数量不能小于0或大于原数量', trigger: 'onBlur' }
        //     ]
    ...
    /** 过账 */
    async handleConfirm() {
      // if (this.tableData.some((item) => !/^[a-zA-Z]|^2/.test(item.matnr || item.idnrk))) {
