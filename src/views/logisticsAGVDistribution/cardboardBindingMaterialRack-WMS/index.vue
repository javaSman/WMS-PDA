...
export default {
  name: 'CardboardBindingMaterialRackWMS',
  ...
  methods: {
    ...
    BoxNoEnter() {
      WMSAPI.get('business/webapi', { boxNo: this.form.boxNo }, 'GetBox').then((res) => {
        if (res && res.success) {
          if (res.result.materialShelfNo === null || this.form.materialShelfNo === '' || res.result === null || res.result.materialShelfNo === '') {
            this.confirmText = '绑定'
            _showFailToast('请输入料架')
            this.getFocus('materialShelfNo')
          } else {
            this.form[this.form.materialShelfNo] = res.result.materialShelfNo
            // console.log(this.form[this.form.materialShelfNo])
            this.$set(this.form, 'materialShelfNo', this.form[this.form.materialShelfNo])
            this.confirmText = '解绑'
          }
        }
      })
    ...
    handleConfirm() {
      this.$refs.formComponent.$refs.refForm.validate().then(() => {
        ...
        if (this.confirmText === '绑定') {
          ...
          WMSAPI.post(
            'business/webapi',
            { boxNo: this.form.boxNo, operatorNo: user.userName, operatorName: user.name },
            'UpdateBoxMaterialShelfNo'
          ).then((res) => {
            if (res && res.success) {
              _showSuccessToast(res.message)
