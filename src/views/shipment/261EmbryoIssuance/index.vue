  ...
  <div>
    ...
    <van-sticky :offset-top="46">
      <van-grid direction="horizontal" :column-num="4" :border="false" style="border-top: 1px solid #ccc; border-bottom: 1px solid #ccc">
        ...
        <van-grid-item>
          <van-checkbox v-model="selectAll" shape="square" :disabled="true" @click="handleSelectAll">全选</van-checkbox>
        </van-grid-item>
      </van-grid>
    </van-sticky>
    <!-- 列表 -->
    <TableVue ref="table" :disabled-chk="true" :table-data.sync="tableData" :table-column="formateTableColumn" :selection.sync="selection" />
    <!-- 底部按钮 -->
...
<script>
// 261发料-胚料发料
const cachedName = 'shipment.261EmbryoIssuance'
        ...
        if (item.prop === 'erfmg') {
          return {
            ...
            rules: [
              ...
              { validator: this.validateQty, message: '不能小于0或者为空', trigger: 'onBlur' }
            ]
          }
        } else {
          return item
  ...
  methods: {
    ...
    async barcodeEnter(val) {
      ...
      if (res.success) {
        ...
        if (!this.computedFormValue()) {
          ...
          this.form = { ...this.form, ..._data.title, bwart: '261' }
    ...
    async handleConfirm() {
      try {
        ...
        _list = _list.map((item) => ({ ...item, ...this.extraParams }))
        let _data = {
          postDate,
          title: _form,
          data: _list
