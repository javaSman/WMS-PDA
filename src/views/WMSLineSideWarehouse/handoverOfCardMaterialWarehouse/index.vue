<template>
  <div>
    ...
    <UserAuth tips-msg="请输入接收人" :visible.sync="userAuthVisible" :extra.sync="extraParams" @comfir="authComfir" />
  </div>
</template>
...
import { GetBoxDetail, GetLocationInfo, GetUserWarehouse } from '@/api/common'
...
export default {
  name: 'HandoverOfCardMaterialWarehouse',
  components: {
    FormVue,
    TableVue,
    // VirtualTable,
  ...
  watch: {
    ...
    multiList: function (val) {
      ...
      console.log('selection', list.length)
  ...
  methods: {
    ...
    async handleConfirm() {
      ...
      try {
        ...
        _list = _list.map((item) => ({
          ...
          targetLocationID: this.form.receiveArea,
          targetBoxID: this.form.wmsTools
        }))
        // 处理接口参数
        ...
        let _data = fomrmatShipmentSubmitParams(
          ...
          'out',
          this.extraParams,
          this.form.receiveWarehouse,
          undefined,
          'materialDesc',
          postDate
        )
        console.log('授权参数', this.extraParams)
        console.log(_data)
        // 调用接口
    ...
    async getWareHouseListAndUpdate() {
      ...
      if (res.success) {
        ...
        if (res.datas.length > 0) {
          ...
          // tips:接收仓库也需要默认选中一个
          this.$set(this.form, 'receiveWarehouse', res.datas[0].warehouseNo)
        }
      }
    },
    ...
    areaEnter(val) {
      ...
      GetLocationInfo(val).then((res) => {
        if (res.success) {
          _this.$set(_this.form, 'receiveWarehouse', res.locationInfo.warehouseNo)
        } else {
          _showFailToast(res.message)
        }
      })
    ...
    initConfig() {
      ...
      this.formList.forEach((item) => {
        ...
        if (item.prop === 'receiveArea') {
          ...
          areaInputRef = _this.getVueComponentByProp('receiveArea')
        }
      })
    },
    ...
    handleClear() {
      ...
      this.form = {
        ...
        receiveWarehouse: this.form.warehouseNo
      }
      this.show = false
      this.fousOfFirst()
    }
